...
import com.example.demo.constant.BeanConstants;
...
//    @Detect
    ...
    SqlLogic(ConnectionHelper connectionHelper,
     ...
     * @throws SQLException
     */
    private void fillBean(Bean bean, boolean fetch, ApiConnection connection) throws SQLException {
...
//            apiConnection = connectionHelper.getConnection(bean.getRequest_db());
                        ...
                        if (!connectionHelper.isEmpty(query)) {
                            ...
                            if (result) {
                                if (!alreadyEntered) {
                                    ...
                                    int count = 0;
                                    // todo: sql 查询返回的行数最多是 1000 行；此处应该根据 DBMT 安全规则设置
    ...
    public void fillBean(Bean bean, boolean fetch, String host, String port, String user, String pass) throws SQLException{
        ApiConnectionImpl connection = dbConnectionService.buildConnection(host, port, user, pass, bean.getRequest_db());
        fillBean(bean, fetch, connection);
    }

    public void fillBean(Bean bean, boolean fetch) {
        return;
    }
